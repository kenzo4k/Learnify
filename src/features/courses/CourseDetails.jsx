...
>>>>>>> e73cd45 (finally Karim's Push)
      ...
      try {
        const courseResponse = await fetch(
          `https://course-management-system-server-woad.vercel.app/api/courses/${id}`
        );
...
=======
        ...
        if (user && token) {
          const statusResponse = await fetch(
            `https://course-management-system-server-woad.vercel.app/api/enrollment-status?courseId=${id}`,
            {
              headers: { authorization: `Bearer ${token}` },
>>>>>>> e73cd45 (finally Karim's Push)
            }
          ...
          const countResponse = await fetch(
            `https://course-management-system-server-woad.vercel.app/api/my-enrollment-count`,
            {
              headers: { authorization: `Bearer ${token}` },
            }
          );
  ...
  const handleEnrollmentToggle = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://course-management-system-server-woad.vercel.app/api/enrollments/toggle",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-gray-800 rounded-lg shadow-xl border border-gray-700 p-6">
              <div className="flex flex-wrap gap-2 mb-4">
                <span className="badge badge-primary bg-indigo-500 text-white">
                  {course?.category}
                </span>
                <span className="badge badge-secondary bg-purple-500 text-white">
                  {course?.level}
                </span>
                <span className="badge badge-accent bg-cyan-500 text-white">
                  {course?.language}
                </span>
                {course?.featured && (
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm text-gray-400">
                  {course?.rating || 0}/5 ({course?.reviews?.length || 0}{" "}
                  reviews)
                </span>
              </div>
            </div>
              ...
              <div className="bg-gray-800 rounded-lg shadow-xl border border-gray-700 p-6">
                ...
                <div className="space-y-3">
                  {course.curriculum.map((item, index) => (
                    <div
                      key={index}
                      className="border-l-4 border-blue-500 pl-4 bg-gray-700 p-3 rounded-r-lg"
                    >
            ...
            <div className="bg-gray-800 rounded-lg shadow-xl border border-gray-700 p-6 sticky top-6">
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-400">Author:</span>
                  <span className="font-medium text-white">
                    {course?.author?.name || "Not available"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Total Enrolled:</span>
                  <span className="font-medium text-white">
                    {course?.enrollmentCount} / {course?.seats}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Seats Left:</span>
                  <span
                    className={`font-medium ${
                      seatsLeft <= 5 ? "text-red-400" : "text-green-400"
                    }`}
                  >
                    {seatsLeft > 0 ? seatsLeft : 0}
                  </span>
              ...
              <div className="mb-6">
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="text-white">
                    {Math.round(
                      (course?.enrollmentCount / course?.seats) * 100
                    )}
                    %
                  </span>
                </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {course.tags.map((tag, index) => (
                      <span
                        key={index}
                        className="badge badge-outline text-xs border-gray-600 text-gray-300 hover:bg-gray-700"
                      >
                        {tag}
                      </span>
                    ))}
